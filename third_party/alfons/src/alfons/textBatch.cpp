...
#include "third_party/alfons/src/logger/logger.h"
...
void TextBatch::setupRect(const Shape& _shape, const hmm_vec2& _position,
                          float _sizeRatio, Rect& _rect, AtlasGlyph& _atlasGlyph) {

    hmm_vec2 ul = _position + (_shape.position + _atlasGlyph.glyph->offset) * _sizeRatio;
    _rect.x1 = ul.X;
    _rect.y1 = ul.Y;
...
void TextBatch::drawShape(const Font& _font, const Shape& _shape,
                          const hmm_vec2& _position, float _scale,
...
void TextBatch::drawTransformedShape(const Font& _font, const Shape& _shape,
    ...
    // return hmm_vec4(atlasGlyph.glyph->u1,
    //     atlasGlyph.glyph->u2,
    //     atlasGlyph.glyph->v1,
    //     atlasGlyph.glyph->v2);
}

hmm_vec2 TextBatch::draw(const LineLayout& _line, hmm_vec2 _position, LineMetrics& _metrics) {
    return draw(_line, 0, _line.shapes().size(), _position, _metrics);
}

hmm_vec2 TextBatch::drawShapeRange(const LineLayout& _line, size_t _start, size_t _end,
                                    hmm_vec2 _position, LineMetrics& _metrics) {
...
hmm_vec2 TextBatch::draw(const LineLayout& _line, size_t _start, size_t _end,
                          hmm_vec2 _position, LineMetrics& _metrics) {
...
hmm_vec2 TextBatch::draw(const LineLayout& _line, hmm_vec2 _position, float _width, LineMetrics& _metrics) {
