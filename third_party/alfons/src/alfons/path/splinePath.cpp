...
SplinePath::SplinePath(const std::vector<hmm_vec2>& points)
    : closed(false) {
    add(points);
}

void SplinePath::add(const std::vector<hmm_vec2>& newPoints) {
...
void SplinePath::add(const hmm_vec2& point) {
...
const std::vector<hmm_vec2>& SplinePath::getPoints() const {
    return points;
}
...
void SplinePath::flush(SplineType type, std::vector<hmm_vec2>& path, float tol) const {
