...
#include "third_party/alfons/src/alfons/path/splinePath.h"
#include "third_party/alfons/src/alfons/utils.h"
...
LineSampler::LineSampler(const std::vector<hmm_vec2>& points)
    : mode(Mode::tangent)
{
    add(points);
}

void LineSampler::add(const std::vector<hmm_vec2>& newPoints)
{
...
void LineSampler::add(const hmm_vec2& point)
{
    if (points.empty()) {
        lengths.push_back(0);
    } else {
        hmm_vec2 delta = point - points.back();

        // Ignore zero-length segments
        if (delta == HMM_Vec2(0, 0))
            return;

        lengths.push_back(lengths.back() + HMM_Length(delta));
    }
...
bool LineSampler::get(float _offset, hmm_vec2& _position, float& _angle) const
{
    float length = getLength();
    ...
    if (length > 0) {
        ...
        _position = hmm_vec2 { 0 };
        _angle = 0;
        return false;
    }
}
hmm_vec2 LineSampler::offset2Position(float offset) const
{
    float length = getLength();
...
{
    hmm_vec2 gradient = offset2Gradient(offset, sampleSize);

    /*
     * We use an epsilon value to avoid degenerated results in some cases
     * (E.g. close to 180 degree diff. between two segments)
     */
    if (HMM_Length(gradient) > 1) {
        return atan2(gradient.Y, gradient.Y);
    } else {
        return offset2Angle(offset);
    }
}

hmm_vec2 LineSampler::offset2Gradient(float offset, float sampleSize) const
{
    hmm_vec2 pm = offset2Position(offset - sampleSize * 0.5f);
    hmm_vec2 pp = offset2Position(offset + sampleSize * 0.5f);
 ...
 */
bool LineSampler::findClosestPoint(const hmm_vec2& input, float threshold, ClosePoint& output) const
{
    ...
    for (int i = 0; i < end; i++) {
        ...
        hmm_vec2 delta = p1 - p0;
        float length = lengths[i1] - lengths[i0];
        float u = HMM_Dot(delta, (input - p0)) / (length * length);

        if (u >= 0 && u <= 1) {
            hmm_vec2 p = p0 + u * delta;
            float mag = HMM_LengthSquared(p - input);
            ...
            float mag0 = HMM_LengthSquared(p0 - input);
            float mag1 = HMM_LengthSquared(p1 - input);
 ...
 * http://paulbourke.net/geometry/pointlineplane
 */
LineSampler::ClosePoint LineSampler::closestPointFromSegment(const hmm_vec2& input, int segmentIndex) const
{
...
void LineSampler::sampleSpline(const SplinePath& path, SplineType type, float tolerance)
{
    ...
    for (size_t i = 1; i < end;) {
        hmm_vec2 delta = points[i] - points[i - 1];
