...
void QuadMatrix::load(const hmm_mat4& matrix)
{
    m = matrix;
}
...
{
    m = HMM_MultiplyMat4(m, HMM_Translate(HMM_Vec3(x, y, z)));
}

void QuadMatrix::scale(float x, float y, float z)
{
    m = HMM_MultiplyMat4(m, HMM_Scale(HMM_Vec3(x, y, z)));
}

void QuadMatrix::rotateX(float a)
{
    m = HMM_MultiplyMat4(m, HMM_Rotate(a, HMM_Vec3(1, 0, 0)));
}

void QuadMatrix::rotateY(float a)
{
    m = HMM_MultiplyMat4(m, HMM_Rotate(a, HMM_Vec3(0, 1, 0)));
}
...
void QuadMatrix::rotateZ(float a)
{
    ...
    m30 = r30;
    m31 = r31;
    // m = glm::rotate(m, a, hmm_vec3(0, 0, 1));
}
...
hmm_vec3 QuadMatrix::transform(float x, float y) const
{
    float x00 = x * m00;
    float x10 = x * m10;
    float x20 = x * m20;
    ...
    return HMM_Vec3(x00 + y01 + m03, x10 + y11 + m13, x20 + y21 + m23);
}
