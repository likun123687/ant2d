...
bool Atlas::addRect(int w, int h, int* rx, int* ry)
{
    ...
    for (size_t i = 0; i < nodes.size(); i++) {
        ...
        if (y != -1) {
            if ((y + h < besth) || ((y + h == besth) && (nodes[i].width < bestw))) {
                besti = i;
                bestw = nodes[i].width;
                besth = y + h;
                bestx = nodes[i].x;
...
bool GlyphAtlas::createGlyph(const Font& _font, const GlyphKey& _key, AtlasGlyph& _entry)
{
    ...
    auto glyphItem = atlas->glyphMap.emplace(_key, Glyph(x, y, w + pad * 2, h + pad * 2, HMM_Vec2(gd->x0, gd->y0) - HMM_Vec2(float(pad), float(pad)), HMM_Vec2(w, h) + HMM_Vec2(float(pad * 2), float(pad * 2))));
    _entry.atlas = id;
