...
struct Shape {
    ...
    Shape(uint16_t faceID, uint32_t codepoint, hmm_vec2 offset,
...
public:
    ...
    {
        if (_start > _end || _end > shapes().size()) {
            return;
        }
    ...
    hmm_vec2 getOffset(Alignment alignX, Alignment alignY) const
    {
        return HMM_Vec2(offsetX(alignX), offsetY(alignY));
    }
    ...
    float offsetY(Alignment align) const
    {
        ...
        case Alignment::middle:
            return m_middleLineFactor * (m_metrics.ascent - m_metrics.descent) * m_scale;
        case Alignment::top:
            return +ascent();
